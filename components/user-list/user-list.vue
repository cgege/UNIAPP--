<template>
	<view class="p-2 flex align-center border-bottom border-light-secondary " hover-class="bg-light">
		<image style="height: 100rpx; width: 100rpx;" class="rounded-circle mr-2" :src="item.avatar" mode=""></image>
		<view class="flex flex-column flex-1">
			<text class="font-md text-dark">{{item.username}}</text>
			<uni-badge :text="item.age" :type="item.sex===1?'error':'primary'" size="small">
				<text v-if="item.sex>0" :class="item.sex===1?'icon-nv':'icon-nan'" class="iconfont  text-white font-sm " style="margin-right: 5rpx;"></text>
			</uni-badge>
		</view>
